...
if __name__ == "__main__":
    print("=== VAYU Wind Forecast Agent ===\n")
    for plant_name, info in WIND_PLANTS.items():
        print(f"Fetching data for {plant_name}...")
        df = fetch_wind_data(info["lat"], info["lon"], days=7)
        print(f"Training model...")
        agent = WindForecastAgent(plant_name, info["rated_mw"])
        agent.train(df)
        print(f"Generating forecast...")
        forecast = agent.predict(df.head(24))
        print(f"\n  24-hour forecast for {plant_name}:")
        print(f"  {'Hour':<6} {'P10':>8} {'P50':>8} {'P90':>8} {'Band':>8}")
        print(f"  {'-'*40}")
        for _, row in forecast.iterrows():
            hr = row["datetime"].strftime("%H:%M")
            print(f"  {hr:<6} {row['p10']:>8.1f} {row['p50']:>8.1f} "
                  f"{row['p90']:>8.1f} {row['band_mw']:>8.1f}")
        print()
